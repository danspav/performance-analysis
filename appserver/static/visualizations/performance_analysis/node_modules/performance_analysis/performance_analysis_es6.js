	...
	*/
	...
	class performance_analysis{
		constructor(granularity, warning_threshold, critical_threshold,down_time_start,down_time_end,time_format,show_legend,showStatusAsText,OKIsLowOrHigh,sortOrder){
			this.items = [];
			this.num_items = 0;
			...
			this.OKIsLowOrHigh = (OKIsLowOrHigh.toLowerCase()=="low") ? "low" : "high";
			//Colour Defaults:
			...
			if (this.sortOrder!="az" && this.sortOrder!="za") {this.sortOrder="default";}
			//----------------
			this.data_item;
			this.current_item;
		}
		...
		setData(data){
			...
			try{
				...
				for (i=0; i<data.rows.length; i++){
					...
					if(date_earliest=="-") { date_earliest=tempTime;}
					if(date_latest=="-") { date_latest=tempTime;}
				...
				for (i=0; i<data.rows.length; i++){
					...
					if(typeof this.items[vizUtils.escapeHtml(data_item[fields["name"]])] === 'undefined') {
						...
						for(k=0;k<current_item.num_buckets;k++){
							...
							temp_bucket.OKIsLowOrHigh = this.OKIsLowOrHigh; 
